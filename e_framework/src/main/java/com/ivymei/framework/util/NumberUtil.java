...
public class NumberUtil {
    ...
    public static String format(Object number) {
        return new DecimalFormat(DEFAULT_MONEY_FORMAT).format(number);
    }
    ...
    public static String format(double number, int fractionDigits) {
        ...
        percentFormat.setMinimumFractionDigits(fractionDigits);
    ...
    /**
     * 将BigDecimal转化为Double，保留两位小数点
     * @param decimal
     * @return
     */
    public static Double formatBigDecimal2Double(BigDecimal decimal) {
        if (decimal == null) {
            decimal = BigDecimal.ZERO;
        }
        String format = df.format(decimal);

        Double doubleValue = Double.parseDouble(format);
        return doubleValue;
    }
